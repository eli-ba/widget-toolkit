...
void BaseWindow::DrawDecoration()
{
	...
	/* Close button */
	if (IsClosable()) currentSurface = closeButton;
	else currentSurface = closeButtonInactive;
	...
	/* Maximize button */
	if (IsMaximizable()) currentSurface = maximizeButton;
	else currentSurface = maximizeButtonInactive;
	...
	/* Minimize button */
	if (IsMinimizable()) currentSurface = minimizeButton;
	else currentSurface = minimizeButtonInactive;
...
{
	if (drawDecoration) DrawDecoration();
	DrawContentSurface();
}
...
void BaseWindow::SetRect(akRect rect)
{
	...
	if ((oldRect.size.width != mRect.size.width) ||
		(oldRect.size.height != mRect.size.height))
	{
		CreateSurface();
		DrawDecoration();
		if (this->Window) {
...
void BaseWindow::SetContentRect(akRect rect)
{
	...
	/* If the size changed we must recreate the surface */
	if ((oldRect.size.width != mContentRect.size.width) ||
		(oldRect.size.height != mContentRect.size.height))
	{
		CreateSurface();
		DrawDecoration();
		if (this->Window) {
...
BaseWindow::~BaseWindow()
{
	...
	if (mContentSurface) SDL_FreeSurface(mContentSurface);
}