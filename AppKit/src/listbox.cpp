	...
	: akControl(rect, parent)
{
	...
	selectionColor.r = 49; selectionColor.g = 106; selectionColor.b = 197;
...
void ListBox_OnMouseMotion(ListBox *listbox, int mouse_x, int mouse_y)
{
	...
	while(p != NULL)
	{
		if(((mouse_x) >= p->info.x + listbox->x) &&
			((mouse_y + listbox->k) >= p->info.y + listbox->y) &&
			((mouse_x) <= p->info.x + p->info.surface_selected->w + listbox->x) &&
			((mouse_y + listbox->k) <= p->info.y + p->info.surface_selected->h + listbox->y))
		{
...
void ListBox_OnMouseButtonDown(ListBox *listbox, int mouse_x, int mouse_y)
{
	...
	if(listbox->of)
	{
		if((mouse_x >= listbox->x + listbox->width - listbox->data[0]->w) &&
			(mouse_y >= listbox->y) &&
			(mouse_x <= listbox->x + listbox->width) &&
			(mouse_y <= listbox->y + listbox->data[0]->h))
		{
		...
		if((mouse_x >= listbox->x + listbox->width - listbox->data[2]->w) &&
			(mouse_y >= listbox->y + listbox->height - listbox->data[2]->h) &&
			(mouse_x <= listbox->x + listbox->width) &&
			(mouse_y <= listbox->y + listbox->height - listbox->data[2]->h + listbox->data[2]->h))
		{
...
{
	if((mouse_x >= listbox->x) &&
		(mouse_y >= listbox->y) &&
		(mouse_x <= listbox->x + listbox->width + listbox->data[4]->w) &&
		(mouse_y <= listbox->y + listbox->height))
	{
		ListBox_ScrollDown(listbox);
	}
}
